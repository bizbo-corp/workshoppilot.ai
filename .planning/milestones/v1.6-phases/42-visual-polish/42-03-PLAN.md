...
must_haves:
  truths:
    ...
    - "Sidebar toggle button shows a subtle hover state"
  artifacts:
...
@.planning/phases/42-visual-polish/42-02-SUMMARY.md
@src/app/layout.tsx
</context>
...
**Sidebar toggle button (`workshop-sidebar.tsx`):**
Find the sidebar collapse/expand toggle button (the chevron/hamburger that opens/closes the step sidebar). Add a subtle hover state: `hover:bg-olive-100 dark:hover:bg-olive-900/30 transition-colors duration-150`. This covers the VISL-06 panel toggle micro-interaction requirement. Note: progress update indicators are already handled by existing step completion visual states (checkmarks, color changes).
...
Run: `npm run build` — should complete without errors.
  ...
  <done>Workshop cards lift with shadow on hover. CTA buttons lift on hover and settle on active. Step sidebar items and toggle button show olive hover states. All transitions are 150ms, subtle and professional.</done>
</task>
...
9. Sidebar toggle button shows hover state
10. Confirm sidebar open/close uses slide-from-edge transition (locked decision: "slide from edge for panels") — verify the shadcn Sidebar component's built-in translate/transition classes are active by inspecting the sidebar wrapper for `transition-transform` or `translate-x` behavior when toggling
</verification>
...
- Sidebar toggle button has hover state
...
- Sidebar slide-from-edge transition confirmed active
