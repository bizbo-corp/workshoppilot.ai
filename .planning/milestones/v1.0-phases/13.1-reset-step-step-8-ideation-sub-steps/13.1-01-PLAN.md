...
7. **ResetStepDialog rendering -- render ONCE outside the mobile/desktop ternary.** The step-container has a top-level structure like:
   ```tsx
   <div className="flex h-full flex-col">
     {isMobile ? (/* mobile layout */) : (/* desktop layout */)}
   </div>
   ```
   Place `<ResetStepDialog>` AFTER the mobile/desktop ternary but still INSIDE the outer `<div>`. This ensures exactly one dialog instance regardless of layout:
   ```tsx
   <div className="flex h-full flex-col">
     {isMobile ? (/* mobile layout */) : (/* desktop layout */)}
     <ResetStepDialog
   ...
   ```
   Do NOT add ResetStepDialog inside both mobile and desktop branches. One instance outside the ternary serves both layouts because Dialog renders as a portal overlay.
  </action>
  <verify>
...
- Click "Reset" and verify confirmation dialog appears with step name and destructive button
  ...
  <done>
Reset Step button visible on in-progress/needs_regeneration steps. Clicking it shows confirmation dialog. Confirming calls resetStep server action, clears local state, and refreshes page. Downstream steps cascade-invalidated. ResetStepDialog renders once (outside mobile/desktop ternary) as a portal overlay.
  </done>
</task>
