...
must_haves:
  truths:
    ...
    - "When user has no credits, dialog shows 'Get 1 Credit' CTA linking to pricing page"
...
- Primary CTA (Button, size="lg", className="w-full"): `"Get 1 Credit — $79"` — wraps a Link to `/pricing` (or use `<Link href="/pricing">` styled as a button)
...
The return statement should wrap with `<>` fragment if needed to include both the existing `<div>` and the `UpgradeDialog`.
  ...
  <done>
    ...
    - When hasCredits=false, "Get 1 Credit — $79" CTA links to /pricing
...
- When user has no credits: "Get 1 Credit — $79" links to /pricing
