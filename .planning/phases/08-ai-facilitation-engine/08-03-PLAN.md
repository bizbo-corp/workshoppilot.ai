...
must_haves:
  truths:
    ...
    - "AI explains step purpose to user during Orient phase using step description"
  artifacts:
...
Purpose: This is the integration layer that connects the prompt content (Plan 01) and state tracking (Plan 02) into the existing chat API. After this plan, every chat message will include step-specific instructions, current arc phase guidance, and (during Validate phase) quality criteria in the system prompt. During Orient phase, the AI explains the step's purpose to the user.
...
<task type="auto">
  <name>Task 1: Enhance buildStepSystemPrompt with arc phase, step instructions, and step description</name>
  <files>src/lib/ai/chat-config.ts</files>
  <action>
Update `src/lib/ai/chat-config.ts` to integrate step-specific prompts, arc phase instructions, step purpose explanation, and validation criteria:
...
2. Update `buildStepSystemPrompt` signature to accept `arcPhase: ArcPhase` and `stepDescription: string` as new parameters:
   ```
   buildStepSystemPrompt(stepId, stepName, arcPhase, stepDescription, persistentContext, summaries)
   ```
...
3. Restructure the prompt assembly:
   ...
   - During Orient phase (arcPhase === 'orient'), add step purpose explanation to the role section: "This step's purpose: {stepDescription}. Explain this purpose to the user in your opening message so they understand what they'll accomplish and why it matters in the design thinking process." This satisfies AIE-03 (step purpose explanation).
...
5. Re-export ArcPhase type from this module for convenience: `export type { ArcPhase } from './prompts/arc-phases'`
  ...
  <verify>
`npx tsc --noEmit` passes. Grep confirms buildStepSystemPrompt now accepts 6 parameters (stepId, stepName, arcPhase, stepDescription, persistentContext, summaries). Grep confirms imports from all three prompt modules. Grep confirms stepDescription appears in the prompt assembly logic with a conditional on 'orient' phase.
  </verify>
  <done>
buildStepSystemPrompt produces system prompts that include step-specific instructions, arc phase guidance, step purpose explanation during Orient phase, and (during validate phase) quality criteria. Existing context injection is preserved. AIE-03 step purpose requirement is satisfied.
  </done>
</task>

<task type="auto">
  <name>Task 2: Wire arc phase and step description into chat API route</name>
  <files>src/app/api/chat/route.ts</files>
  <action>
Update `src/app/api/chat/route.ts` to read arc phase from database, look up step description, and pass both to the system prompt builder:

1. Add imports:
   - `import { getCurrentArcPhase } from '@/lib/ai/conversation-state'`
   - `import { STEPS } from '@/lib/workshop/step-metadata'` (for step description lookup)

2. After the existing `assembleStepContext` call, add arc phase retrieval and step description lookup:
   ```
...
3. Update the `buildStepSystemPrompt` call to pass `arcPhase` and `stepDescription`:
   ```
...
Note: Arc phase transitions are NOT handled in this plan. The AI will always receive the current arc phase in its system prompt. Phase transitions (when the AI suggests "Ready to see a draft?" and the conversation moves from Gather to Synthesize) will be handled by a future mechanism -- either the client sending a transition request, or the AI detecting it. For now, the system starts at 'orient' and the system prompt reflects that. This is intentional: getting the prompts right is step 1, transitions are step 2 (gap closure or Phase 11+).
  ...
  <verify>
`npx tsc --noEmit` passes. Grep confirms getCurrentArcPhase is imported and called in route.ts. Grep confirms STEPS is imported from step-metadata.ts. Grep confirms buildStepSystemPrompt is called with 6 arguments. Run `npm run build` to verify no build errors.
  </verify>
  <done>
Chat API reads arc phase from database and step description from metadata, passes both to buildStepSystemPrompt, and produces arc-phase-aware system prompts. During Orient phase, the AI is instructed to explain the step's purpose. Every AI response now includes step-specific instructions and current arc phase guidance.
  </done>
</task>
...
- Chat API route imports STEPS and looks up stepDescription
- buildStepSystemPrompt accepts arcPhase and stepDescription parameters and injects both
- During Orient phase, step description is included in the role section with instruction to explain purpose
