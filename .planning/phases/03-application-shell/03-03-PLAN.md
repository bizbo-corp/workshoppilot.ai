...
4. Initialize all 10 workshop_steps records as `'not_started'`:
   - For each of the 10 step definition IDs ('empathize', 'define', 'ideate', 'sense_making', 'persona', 'journey_mapping', 'reframe', 'ideation', 'concept', 'validate'), create a workshopSteps record
   - Set step 1 ('empathize') to `'in_progress'` since user starts there
   - IMPORTANT: Check the actual step definition IDs from the seed script to use correct values
5. Call `redirect(`/workshop/${sessionId}/step/1`)` to send user to Step 1.

Read `scripts/seed-steps.ts` to get the exact step definition IDs used in the database.
...
Verify the function performs: workshop insert, session insert, workshopSteps inserts (10 records), and redirect.
  ...
  <done>Server Action `createWorkshopSession` creates workshop + session + 10 workshop_steps in DB, then redirects to `/workshop/[sessionId]/step/1`. Works for both authenticated and anonymous users.</done>
</task>
...
- Server action creates workshop + session + 10 steps in DB
...
<success_criteria>
Users can click "Start Workshop" on the landing page, which creates a workshop session in the database and redirects them to Step 1. The flow works for both anonymous and authenticated users. A brief loading screen appears during creation.
</success_criteria>
