...
<task type="auto">
  ...
  <action>
    ...
    3. Create `src/middleware.ts` at project root (NOT inside src/app):
       ...
       - ============================================================
         PHASE 3 HANDOFF: WORKSHOP STEP ROUTE PROTECTION (CRITICAL)
         ============================================================
         Workshop step routes (/workshop/[id]/step/[N]) do NOT exist yet
         (they are created in Phase 3). When Phase 3 adds these routes,
         the middleware MUST be updated as follows:

         Per user decision (LOCKED):
           - PUBLIC routes (no auth required): workshop steps 1, 2, 3
             Add to isPublicRoute: '/workshop/:path*/step/1', '/workshop/:path*/step/2', '/workshop/:path*/step/3'
           - PROTECTED routes (auth required): workshop steps 4-10
             Add to isProtectedRoute: '/workshop/:path*/step/(4|5|6|7|8|9|10)'
           - The auth wall modal triggers AFTER step 3 when user tries to advance to step 4

         This is the boundary between anonymous and authenticated workshop usage.
         Phase 3 planner/executor: search for "PHASE 3 HANDOFF" in this file.
         ============================================================

         Note: The exact route pattern syntax (e.g., regex vs path params) should
         match whatever pattern Phase 3 uses for workshop routes. The key contract
         is: steps 1-3 = public, steps 4-10 = protected.
       ...
       - IMPORTANT: Add a clearly visible block comment in the middleware source code itself
         (not just in this plan) marking the Phase 3 handoff. Use this exact format in the code:
  ...
  <verify>
    ...
    - Middleware source code contains "PHASE 3 HANDOFF" comment block
  </verify>
  <done>ClerkProvider wraps the application. Middleware protects /dashboard and /admin routes, redirects unauthenticated users to landing page, and redirects authenticated users from landing to dashboard. Middleware contains clearly marked Phase 3 handoff comment for workshop step route protection (steps 1-3 public, steps 4-10 protected). Next.js 16.1.1 is used (post-CVE-2025-29927 patch).</done>
</task>
...
4. Middleware.ts contains "PHASE 3 HANDOFF" comment block documenting workshop step route protection requirements
5. ClerkProvider wraps app in layout.tsx
6. Schema exports include users table and User/NewUser types
</verification>
...
- Middleware contains explicit Phase 3 handoff block: "PHASE 3 HANDOFF: WORKSHOP STEP ROUTE PROTECTION" with exact route patterns (steps 1-3 public, steps 4-10 protected)
