...
must_haves:
  truths:
    ...
    - "Canvas/right panel shows skeleton while canvas or step content loads"
...
Verify `StepTransitionWrapper` is imported and used in `step-container.tsx`.
  ...
  <done>Step navigation produces a subtle 150ms fade-in. Both chat and canvas panels transition together. No jarring content swap.</done>
</task>
...
Run: `grep -n 'animate-pulse' src/components/workshop/chat-skeleton.tsx src/components/dashboard/workshop-grid-skeleton.tsx` — should return 0 matches (no pulse animation).
  ...
  <done>Chat panel shows message-shaped static gray skeletons during load. Dashboard shows card-shaped static gray skeletons during load. No shimmer or pulse animation on any skeleton.</done>
</task>
