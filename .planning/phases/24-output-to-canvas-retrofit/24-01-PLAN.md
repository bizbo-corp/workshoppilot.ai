  ...
  <action>
    ...
    **Replace** the `sense-making` config entry:
    ...
    - Define `empathyZoneConfig` with 6 zones (classic empathy map layout):
      - 4 quadrants in classic 2x2 grid (Says/Thinks top row, Feels/Does bottom row), ~800px wide x 680px tall total, gap between them:
        - `says`:   `{ bounds: { x: -420, y: -700, width: 400, height: 330 }, label: 'Says',   color: '#94a3b8' }` (slate, top-left)
        - `thinks`: `{ bounds: { x:   20, y: -700, width: 400, height: 330 }, label: 'Thinks', color: '#a1a1aa' }` (zinc, top-right)
        - `feels`:  `{ bounds: { x: -420, y: -350, width: 400, height: 330 }, label: 'Feels',  color: '#a3a3a3' }` (neutral, bottom-left)
        - `does`:   `{ bounds: { x:   20, y: -350, width: 400, height: 330 }, label: 'Does',   color: '#9ca3af' }` (gray, bottom-right)
