...
### 🚧 v1.8 Onboarding + Payments (In Progress)
...
**Success Criteria** (what must be TRUE):
  1. Steps 1-6 load for any authenticated user with zero credits — no paywall check fires
...
**Success Criteria** (what must be TRUE):
  1. Steps 7-10 show a lock badge in the stepper when `creditBalance === 0` and `creditConsumedAt` is not set for the current workshop — users see the gate coming before they reach it
...
**Success Criteria** (what must be TRUE):
  1. The pricing page displays three tiers: Single Flight ($79, 1 credit), Serial Entrepreneur ($149, 3 credits), and Agency (Contact Sales) — the old v1.5 tiers are gone
  2. The Agency tier shows a "Contact Sales" button that opens a mailto or contact form — no self-serve billing flow is triggered
  ...
  4. The full purchase path works end-to-end: pricing page → Stripe Checkout → success page with credits applied → return to dashboard showing updated balance → Step 7 accessible in an existing workshop
**Plans**: TBD
