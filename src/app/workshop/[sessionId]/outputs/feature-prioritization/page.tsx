...
interface FeaturePrioritizationPageProps {
  params: Promise<{ sessionId: string }>;
}

export default async function FeaturePrioritizationPage({ params }: FeaturePrioritizationPageProps) {
  ...
  // Load saved feature prioritization
  const rows = await db
    .select()
    .from(buildPacks)
  ...
  let savedState: FeaturePrioritizationState | null = null;

  if (jsonRow) {
    try {
      savedState = JSON.parse(jsonRow.content!) as FeaturePrioritizationState;
    } catch {
      // Invalid JSON, treat as no saved state
    }
  }

  // Check if journey map exists (needed for generation)
  const journeyRows = await db
    .select()
    .from(buildPacks)
    .where(and(eq(buildPacks.workshopId, workshop.id), like(buildPacks.title, 'Journey Map:%')));

  const hasJourneyMap = journeyRows.some((r) => r.formatType === 'json' && r.content);

  // Check if Step 9 is complete
