...
function buildImagePrompt(persona: {
  ...
  const style =
    'Black and white flat illustration caricature portrait. Clean confident lines, minimal shading, editorial illustration style. Head and shoulders composition. White background only.';

  const subject = [
    persona.name || 'A person',
    persona.age ? `age ${persona.age}` : '',
    persona.job ? `working as ${persona.job}` : '',
  ]
    .filter(Boolean)
    .join(', ');
  ...
  return `${style} ${subject}. ${personality} ${expression} ${context}`.trim();
}
