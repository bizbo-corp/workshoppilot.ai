...
export function EditableColumnHeader({ label, onSave, className = '' }: EditableColumnHeaderProps) {
  ...
  return (
    <div
      ...
      className={`nodrag nopan px-2 py-0.5 text-xs font-semibold cursor-pointer hover:bg-[#8a9a5b]/10 dark:hover:bg-neutral-olive-700/80 hover:rounded transition-colors text-[#4a5a32] dark:text-neutral-olive-200 truncate max-w-[140px] ${className}`}
    >
      {label}
    </div>
  );
