 ...
 * Inserts all design thinking step definitions.
 * `order` mirrors src/lib/workshop/step-metadata.ts: challenge = 0 (pre-workshop
 * setup, unnumbered), the ten workshop steps are 1 (stakeholder-mapping) …
 * 10 (validate). Safe to run multiple times (uses onConflictDoUpdate).
 */
...
async function seedStepDefinitions() {
  ...
  try {
    ...
    console.log(`\n✅ Successfully seeded all ${STEP_DEFINITIONS.length} step definitions`);
    process.exit(0);
  } catch (error) {
